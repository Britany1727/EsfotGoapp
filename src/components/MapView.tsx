...
function LugarListItem({ item, index, onPress, onDelete }: { item: any; index: number; onPress: () => void; onDelete: () => void }) {
  ...
  return (
    <Animated.View
      // NUEVA ANIMACIÓN DE ENTRADA: Efecto Pop Up elástico en cascada secuencial
      entering={FadeInDown.duration(650)
        .delay(index * 90) // Desfase temporal sutil entre tarjetas
        .springify()       // Convierte la transición en física de muelle
        .damping(13)       // Controla la elasticidad del rebote final
        .mass(0.8)         // Reduce la masa del objeto para una subida ágil
      }
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Animated.View entering={FadeInDown.duration(600).springify()} style={styles.emptyCard}>
                <LottieView
                  source={require('../../src/assets/lottie/No Search result.json')} 
                  autoPlay
                  loop
...
const styles = StyleSheet.create({
  ...
  searchContainer: {
    ...
    borderWidth: 1.5,
    borderColor: 'rgba(59, 130, 246, 0.25)', // Glow sutil para Android
    borderRadius: 12,
  ...
  lugarCard: {
    ...
    overflow: 'hidden', 
    borderWidth: 1.5,
    borderColor: 'rgba(59, 130, 246, 0.2)', // Línea de contorno azul delgada

    shadowColor: '#3b82f6', // Tono azul estético para iOS
